...
struct pair ask_list(void)
{
	struct pair thermal_devices_list = get();
}

struct thermal_zone_device * ask_device(void)
{
	struct pair thermal_devices_list = ask_list();
	int sz = thermal_devices_list.sz;
	return thermal_devices_list.ptr[sz - 1];
}
